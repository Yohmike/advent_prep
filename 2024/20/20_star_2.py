...
def parse_input_and_solve(filename: str) -> int:
    with open(filename) as file:
        ...
        smaller = 0
        cheatsheet = defaultdict(int)
        threshold = 100
        max_picosends_cheat = 20
        for i in range(0, len(route)):
            for j in range(i + threshold - 1, len(route)):
                fake_start = route[i]
                fake_end = route[j]
                md = manhattan_distance(fake_start, fake_end)
                if md <= max_picosends_cheat:
                    saved = distances[fake_end] - distances[fake_start] - md
                    if saved >= threshold:
                        smaller += 1
                        cheatsheet[saved] += 1
