...
class File(object):
    ...
    @classmethod
    def traverse_size(self, node, size_dir):
        ...
        if node.type == FileType.DIR:
            for child in node.children:
                if child.type == FileType.DIR:
                    self.traverse_size(child, size_dir)
                    # print(sum, node.name)
            size_dir[node.size] = node.name
        return sum
...
def parse_input_and_solve(filename):
    ...
    current_space_used = total_space - root.size
    size_dir = {}
    File.traverse_size(root, size_dir)
    sorted_keys = list(sorted(size_dir.keys()))
    print(sorted_keys)
    print(root)

    for key in sorted_keys:
        if current_space_used + key > unused_space_needed:
            return key
    print(size_dir.keys())
