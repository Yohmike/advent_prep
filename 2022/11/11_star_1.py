...
def throw_items(monkeys, monkey_id):
    monkey = monkeys[monkey_id]
    for item in monkey.items:
        monkey.items_inspected += 1
        worry_level = worry_operation(item, monkey.operation) // 3
...
def simulate_rounds(monkeys, rounds = 20):
    for round in range(0, rounds):
        for monkey in monkeys:
            throw_items(monkeys, monkey.id)
