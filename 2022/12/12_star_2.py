...
def to_adj_list(matrix):
    ...
    return sources, destination, adj_list, chars
...
def parse_input_and_solve(filename):
    ...
    sources, destination, adj, chars = to_adj_list(height_map)
    min_dist = 10000000
    print(len(sources))
    for i, source in enumerate(sources):
        distances, parents = djikstra(adj, len(adj), source)
        print(i, distances[destination])

        min_dist = min(min_dist, distances[destination] - 2)
